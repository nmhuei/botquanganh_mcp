...
mcp = FastMCP(
    ...
    version=VERSION,
    instructions=(
        "Host-only MCP server with built-in CTF Toolkit capabilities. "
        ...
        "Step 3: Call 'host_workspace_status()' to inspect active workspace health, quota, and state. "
        "Only after binding a workspace may you use host_run_command and file tools. "
        ...
        "\n--- CTF TOOLKIT PLAYBOOK & SOLVING FLOW ---\n"
        "When solving CTF challenges, follow the CTF Toolkit Playbook (read via host_knowledge(section='guide') "
        "or knowledge/CTF_TOOLKIT_PLAYBOOK.md). Adhere strictly to these principles:\n"
        "1. EVIDENCE-FIRST: Never declare solved without local deterministic reproduction. Candidate flag states: "
        "CANDIDATE -> VERIFIED -> REJECTED.\n"
        "2. NO AUTOMATED FLAG SUBMISSION: Never automatically submit flags to external CTF scoreboards. Always save "
        "found flags locally using host_save_note and report them with deterministic proof for the user to review.\n"
        "3. ARTIFACT TRIAGE: ALWAYS call 'ctf_triage_artifact' FIRST when identifying files or binaries (magic/format, "
        "architecture, NX/PIE/Canary/RELRO, entropy, suspicious strings). Do NOT run file/checksec/strings via shell.\n"
        "4. FAST OFFLINE TRANSFORMS: ALWAYS use 'ctf_transform' for base64, hex, url, rot13, gzip, zlib, and XOR. "
        "Do NOT write throwaway python scripts or shell pipelines for supported transforms.\n"
        "5. PWN OFFSET RESOLUTION: Use 'ctf_pattern(action=\"create\", length=...)' and 'ctf_pattern(action=\"offset\", value=...)' "
        "for instant De Bruijn cyclic sequence generation and crash offset calculation (pwntools compatible).\n"
        "6. HASH IDENTIFY & DIGESTS: Use 'ctf_hash_tool(action=\"identify\", value=...)' to identify hash types/modes, "
        "and 'ctf_hash_tool(action=\"compute\", value=...)' to compute standard digests (MD5, SHA1, SHA256, NTLM, CRC32).\n"
        "7. WEB TARGETS: When user explicitly authorizes an HTTPS CTF URL, use 'ctf_fetch_url' for single read-only GET. "
        "Never scan, crawl, or fuzz without explicit scope.\n"
        "8. PROGRESS LOGGING: Record candidate hypotheses, probe results, and candidate flags using host_save_note."
    ),
)
