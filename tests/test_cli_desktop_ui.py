...
def test_ui_command_uses_desktop_window(monkeypatch):
    called = []
    monkeypatch.setattr("app.cli.main.run_desktop_ui", lambda ctx: called.append(ctx) or 0)
...
def test_ui_detach_dispatches_to_background_launcher(monkeypatch, capsys):
    started = []
    monkeypatch.setattr("app.cli.main.graphical_session_available", lambda: True)
    monkeypatch.setattr(
        "app.cli.main.launch_desktop_ui_detached",
